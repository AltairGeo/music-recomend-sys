...
async def get_random_track(
    ...
    return ListTracksResult(total=len(tracks), tracks=tracks)
...
async def similar_tracks(
    ...
    return SimilarTracksResult(
        tracks=[
            TrackSimilar(track=TrackDTO.from_domain(track), score=score)
...
@router.post(
    "/",
    response_model=SimilarTracksResult,
    description="Поиск похожих треков в базе данных на загруженный пользователем",
)
async def upload_track(
    ...
    similar = await rec_service.get_similar_by_vector(embedding, k=10)
